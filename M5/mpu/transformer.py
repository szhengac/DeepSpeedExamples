...
class BertParallelSparseSelfAttention(torch.nn.Module):
    """Parallel sparse self-attention layer for BERT.

    Sparse Self-attention layer takes input with size [b, s, h] where b is
    the batch size, s is the sequence lenght, and h is the hidden size
    and creates output of the same size.
    Arguments:
        hidden_size: total hidden size of the layer (h).
        ...
        output_parallel: If true, no all-gather is done on the output and
                         the output values will be per partition.
        sparsity_config: sparsity config for sparse self-attention
                         see https://github.com/microsoft/DeepSpeed/blob/master/deepspeed/ops/sparse_attention/sparsity_config.py
    We use the following notation:
        h: hidden_size
        n: num_attention_heads
        p: number of partitions
    ...
    def __init__(self,
        ...
        super(BertParallelSparseSelfAttention, self).__init__()
        # Input configuration.
        self.hidden_size = hidden_size
        self.num_attention_heads = num_attention_heads
        self.output_parallel = output_parallel
